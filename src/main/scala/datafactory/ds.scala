...
class ds(dalist:Vector[da]) extends Descritive 
    ...
    val sd=dalist.map(_.sd)
    
    
    lazy val combi:Vector[Vector[da]]=dalist.combinations(2).toVector
...
//Operation	
	...
	def ts=new tsds(dalist.map(a=>a.ts))
	//時系列データ化
	
	def naming(in:String*)=in.zip(dalist).foreach{a=>a._2.name=a._1} 
	def naming(in:Array[String])=in.zip(dalist).foreach{a=>a._2.name=a._1} 
	//side effect
  
	def resolve:Vector[da]=dalist
	//dsを分解しdaのVectoruenceを返す
	
	def tomat=new m(raw)
	//def mat(direction)={データを行列に変換＝＝行列クラスのインスタンスを返す}
	
	def ::(component:da)=new ds(component+:dalist)
    ...
    def summary={
        mkLine 
        println("name list : ")
        dalist.foreach(x=>println(x.name))
		mkLine
		println("length of each")
        dalist.foreach(a=>println(a.n))
        mkLine
		println("mean : ")
