package scalearn.preprocessing.tokenfactory
...
object JP{
    ...
    def base(Stream:Stream[Token]): Stream[String] =
        Stream.map((x:Token)=>x.getBaseForm)
	//Tokenのリストから基の単語リストを取得
	
    def reading(Stream:Stream[Token]): Stream[String] = 
        Stream.map((x:Token)=>x.getReading)
	//Tokenのリストから読みの単語リストを取得
	
    def resolverF(path:String):Stream[String] =
        mkTokenF(path).map((x:Token)=>x.getBaseForm)
	//Fileのパスから元の単語リストを取得
...
//---------------------------------------------------カリー化や合成の試し
    ...
    def oppcond(limit1:String)(limit2:String) =
        condition( (x:Token) =>
            (!(x.getPartOfSpeech.startsWith(limit1)) 
            && 
            !(x.getPartOfSpeech().startsWith(limit2)))
            )
