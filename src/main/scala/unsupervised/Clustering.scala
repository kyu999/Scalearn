...
object Clustering {
	...
	def reclustering(vectors:ArrayBuffer[Vector[Double]],clusters:IndexedSeq[VectorCluster]) = {
		...
		vectors.foreach{ vect =>
			...
			var maxsim:(VectorCluster,Double) = (clusters.head , Stats.pearRaw(clusters.head.center,vect))
			
			clusters.foreach{ cluster =>
			  
			  val sim:(VectorCluster,Double) = (cluster , Stats.pearRaw(cluster.center,vect))
